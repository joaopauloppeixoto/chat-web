...
export default function SelectedChatInfo({ buttons }: SelectedChatInfoProps) {
  ...
  useEffect(() => {
    if (selectedChat?.chatWithId) {
      api
        .get(`${apiUrl}/account/?guid=${selectedChat.chatWithId}`, {
          headers: {
  ...
  return (
    <div
      className={`flex flex-row justify-between px-7 py-2 ${
        selectedChat ? "bg-secondary text-foreground" : ""
      }`}
    >
      ...
      <div className="my-auto flex flex-row gap-2">{buttons}</div>
    </div>
  );
}
